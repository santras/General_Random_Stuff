...
path="..\Data_new\Other\Solution\\"
path2="..\..\CMEMS_hourly_EVRF07\Fast_check\\"

output_path= "Plots\\"      # Outputpath for images
...
def process_file(filename,sl_variables,sl_variables2,Headers,order,station):
    ...
    output_file=output_path+station.replace(" ", "")+"_plot2.png"                   # HERE OUTPUTFILENAME
    ...
    sl_variables2=sorted(sl_variables2)

   # new_var=[]
    #for ind in range(len(sl_variables)):
    #    if sl_variables[ind][0]>datetime.datetime(2000,1,1):
     #       new_var.append(sl_variables[ind])

    #variables=new_var

    for ind in range(len(sl_variables)):
        if sl_variables[ind][1]>500 or sl_variables[ind][1]<-300:
            sl_variables[ind][1] = np.nan

    values=[]
    for i in range(3):
        values.append([row[i] for row in sl_variables])
    maxi_time=(np.max(values[0]))
    mini_time=(np.min(values[0]))
    maxi = (np.nanmax(values[1]))
    mini=(np.nanmin(values[1]))

    values2=[]
    for i in range(3):
        values2.append([row[i] for row in sl_variables2])

    if maxi_time<np.max(values2[0]):
        maxi_time=np.max(values2[0])
    if mini_time>np.min(values2[0]):
        mini_time=np.min(values2[0])

    if (np.nanmax(values2[1]))>maxi:
        maxi = (np.nanmax(values2[1]))

    if (np.nanmin(values2[1]))<mini:
        mini = (np.nanmin(values2[1]))


    #print(mini,maxi)

    write_plot(values,values2, station, output_file, maxi, mini,maxi_time, mini_time)
...
# Function 6
def write_plot(values, values2, station, outputfile,maxi,mini,tmax,tmin):
    ...
    #plt.plot(values[0], values[1], linestyle="none", marker=".", color="b", markersize=4, label="New Data")
    plt.plot(values2[0], values2[1], linestyle="none", marker=".", color="r", markersize=4, label="CMEMS Data")
    plt.plot(values[0],values[1],linestyle="none",marker=".",color="b", markersize=4,label="New Data")   # marker='.' linestyle='-'
...
def main():
    ...
    for file in glob.glob("*.txt"):                 # Opens all that ends with .txt in the path folder one by one
        if not file in glob.glob("Wlady*"):
            print(file)
            ...
            if not okey:
                ...
                exit()
           # for ind in range(len(sl_variables)):
            #    all_vari.append(sl_variables[ind])
            ...
            process_file(file, sl_variables, sl_variables2, Header_dict, header_order, station)

   # process_file(file,all_vari,Header_dict,header_order,station) # Function 5
