...
# Function 5
def process_file(filename,variables,Headers,order,station):
    ...
    output_file=output_path+station.replace(" ", "")+"_plot.png"                   # HERE OUTPUTFILENAME
    ...
    for ind in range(len(variables)):
        if variables[ind][0]>datetime.datetime(2000,1,1):
            new_var.append(variables[ind])

    #variables=new_var

    variables=sorted(variables)
    for ind in range(len(variables)):
        if variables[ind][1]>500:
            variables[ind][1] = np.nan
    ...
    values=[]
    for i in range(3):
        values.append([row[i] for row in variables])
...
def write_plot(values, station, outputfile,maxi,mini):
    ...
    plt.plot(values[0],values[1],linestyle="none",marker=".",color="b", markersize=4)   # marker='.' linestyle='-'
...
def main():
    ...
    for file in glob.glob("*.txt"):                 # Opens all that ends with .txt in the path folder one by one
        if file not in glob.glob("Klaipeda*"):
            print(file)
