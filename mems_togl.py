...
def check_data(name,sl_variables):
    ...
    if not inorder:
        ...
        #for i in range(3):
        #    transposed.append([row[i] for row in sl_variables])
        #(inorder, inds) = check_listorder(transposed[0],time_difference * 60)  # Function 7

        #if not inorder:
         #   for ind in range(1,len(sl_variables)):
        #        if sl_variables[ind][0]-sl_variables[ind-1][0]<datetime.timedelta(seconds=(time_difference*60)):
        #            need_ave=True
        #            print(" Time differencing needed")
         #           break
        #    else:
        #         need_ave=False
        #
        ...
        #         new_variable.append(sl_variables[index])
            ...
            # For nc files averaging on a separate file
            #transposed=[]
            #for i in range(3):
            #    transposed.append([row[i] for row in new_variable])
            #(inorder, inds) = check_listorder(transposed[0], time_difference * 60)  # Function 7, Just to be sure it's now in order,huoh..
            #if not inorder:
            #    print("Problem with ordering in check_data/Function 6")
