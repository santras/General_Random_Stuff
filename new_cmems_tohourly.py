...
output_path= "..\..\Data_new\CMEMS_hourly_fast_check\\"      # than the .txt data file in this directory nop= no parsing empties
time_difference=60                                  # Time difference in minutes between measurements
max_sealevel_difference=50                          # In cm
check_files=True                          # Normally True, only false if you don't want the check to be made
patching_missing=True                     # Normally True, only false if you don't want the check to be made
...
def check_data(name,sl_variables):
    ...
    missing=0
    var_onlyhour=[]
    for ind in range(1, len(sl_variables)):
...
# File is empty
    ...
    print("Too big jumps: ",len(too_big))
    ...
    #
    #
    #     # If only interested to look at the too big jump points, then run write_files=False
    #     if write_files == False:
    #         print("Too big jump in seal level between measurements: ", len(too_big))
    #         #print("here")
    #         match_ind=[]
